...
const UserPaypal = () => {
    ...
    return (
        <>
            {userPaypalHistory.length === 0 ? (<p>No order available</p>) : (

                userPaypalHistory.map((paypal) => (
                    <>
                        <Card title={`Payment ID: ${paypal.paymentId}`} bordered={true} style={{ width: 400, margin: '20px auto' }}>
                            <Descriptions column={1} bordered>
                                <Descriptions.Item label="User ID">{paypal.userId}</Descriptions.Item>
                                <Descriptions.Item label="Amount">
                                    {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(paypal.money)}
                                </Descriptions.Item>
                                <Descriptions.Item label="Date">
                                    {dayjs(paypal.date).format('YYYY-MM-DD HH:mm')}
                                </Descriptions.Item>
                                <Descriptions.Item label="Title">{paypal.title}</Descriptions.Item>
                            </Descriptions>
                        </Card>
                    </>
                ))
